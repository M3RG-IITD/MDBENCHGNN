...
def tmdnet_calculator(model, **kwargs):
    """Build ASE Calculator directly from deployed model."""
    return TmdnetCalculator.from_deployed_model(model, **kwargs)


class TmdnetCalculator(Calculator):
    """TorchMDnet ASE Calculator.
    ...
    def calculate(self, atoms=None, properties=["energy"], system_changes=all_changes):
        ...
        sample_an = torch.tensor(data["atomic_numbers"].reshape(-1), dtype=torch.int64)
        sample_pos = torch.tensor(data["pos"], dtype = torch.float)
        # import pdb; pdb.set_trace()
        pred_y , pred_dy = self.model(sample_an, sample_pos)
        self.results = {}

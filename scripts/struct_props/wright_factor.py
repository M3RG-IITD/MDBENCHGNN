...
#Example command:
"""
python wright_factor.py \
--traj_path "/home/civil/btech/ce1180169/MDBENCHGNN/example_mdbenchgnn/lips20_small/data/test/botnet_g75.xyz" \
...
def main(args=None):
    ...
    print("Calculating Wright Factor .....")
    ...
    plt.savefig(args.out_path+"/"+args.sys_name+"_"+yaxis_label+"_Wright"+".png")
    
    data = pd.DataFrame({'r': r, 'g_r': g_r, 'g_r_ref': g_r_ref })
    data.to_csv(args.out_path+"/"+args.sys_name+"_"+yaxis_label+"_Wright"+".csv")
    Wright_factor= np.sum((g_r-g_r_ref)**2)/np.sum(g_r_ref**2)
    print("Wright Factor:", Wright_factor*100 ,"%")
    print("Done..., check PDFs at "+args.out_path)    
    #plt.show()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate and plot RDF")
    parser.add_argument("--traj_path", required=True, help="Path to generated trajectory file")
